/**
 * Place cookies on gem cells near pocket boundaries.
 * Cookies are 3-HP blockers that trap the block underneath.
...
export function placeCookiesPass(ctx: LevelGenContext): LevelGenContext | null {
  ...
  for (const pos of candidates) {
    ...
    setCellById(board, cellIdOf(pos.row, pos.col), createObstacle('cookie', { kind: 'block', color: cell.color }));
    placed++;
  }
